<template>
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ isEditing ? t('categories.editCategory') : t('categories.newCategory') }}</h1>
                <p class="text-gray-500 text-sm mt-1">{{ t('categories.descriptionText') }}</p>
            </div>
            <div class="flex gap-3">
                ...
                >
                    ...
                    <span>{{ isEditing ? t('categories.updateCategory') : t('categories.createCategory') }}</span>
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-2 space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h3 class="text-lg font-medium text-gray-900 mb-4 border-b pb-2">{{ t('categories.basicInformation') }}</h3>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col items-center">
                    <h3 class="text-lg font-medium text-gray-900 mb-3">{{ t('categories.photo') }}</h3>
                    <div class="h-24 w-24 rounded-full overflow-hidden border border-gray-200 mb-3">
                        <img v-if="form.image" :src="form.image" :alt="form.name || t('categories.categoryImage')" class="object-cover h-full w-full" @error="onImageError" />
                    ...
                    <div class="flex gap-2 mb-2">
                        <button @click="showUploader = !showUploader" type="button" class="px-3 py-1 text-sm bg-indigo-600 text-white rounded">{{ t('categories.edit') }}</button>
                        <button v-if="form.image" @click="confirmRemoveImage" type="button" class="px-3 py-1 text-sm border rounded">{{ t('categories.remove') }}</button>
                    </div>

                    <p class="text-xs text-gray-400 text-center">{{ t('categories.recommendedSize') }}</p>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">{{ t('categories.structure') }}</h3>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ t('categories.parentCategory') }}</label>
                        <select 
                            v-model="form.parent_id" 
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        >
                            <option :value="null">{{ t('categories.noneRootCategory') }}</option>
                            <option 
                        ...
                        <p class="mt-2 text-xs text-gray-500 italic">{{ t('categories.selectParentIfSubcategory') }}</p>
                    </div>
                </div>
...
const confirmRemoveImage = async () => {
    const result = await swal.confirm(t('categories.confirmRemoveImage'), t('categories.removeImage'));
...
const fetchCategory = async () => {
    ...
    try {
        ...
        swal.error(t('categories.failedToLoadCategory'));
    }
};

const submit = async () => {
    errors.name = !form.name ? t('categories.nameIsRequired') : '';
    errors.slug = !form.slug ? t('categories.slugIsRequired') : '';
    ...
    try {
        ...
        if (error.response?.data?.errors) {
            ...
            swal.error(t('categories.failedToSaveCategory'));
        }
    } finally {
